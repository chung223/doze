...
**一晚要花多少**
輸入「玩幾小時、每注多少、帶多少錢」，直接算出這一晚的<b>期望成本</b>、九成機率的區間、
收在正的機率，以及帶的錢撐不完的機率——並換算成每小時多少，讓你當娛樂費用看。
押錯注區的代價在這裡最直白：4 小時、每注 100，押「大」期望成本 444，押「點數 9」是 <b>3,037</b>。

**桌邊生存指南**
真的走進賭場會遇到的流程：怎麼換籌碼、限紅牌上兩個數字的意思、
什麼時候手不能再碰檯面、派彩的順序、帶多少去、什麼時候該走。
新手卡住的通常不是數學，是流程跟怕出糗——這一節補的是那塊。

**新手提示**
押到高莊家優勢的注區時就地提醒：「點數 9：莊家優勢 18.98%，同桌的大／小只有 2.78%。」
同一種注區只提醒一次，標題列可隨時關掉。

**賠率快問快答**
一輪十題，考三件事：兩個注區<b>哪個划算</b>、這注<b>賠多少</b>、開出這三顆<b>算不算中</b>。
每題答完立刻給出實際數字當解說（「『全圍』莊家優勢 13.89%，『圍骰』16.20%，每押 100 差 2.31 元」）。
題目依**目前生效的賠率表**即時產生——你把賠率改成自己那張桌，練的就是那張桌。

<p align="center">
  <img src="docs/screenshot-quiz.png" width="48%" alt="賠率快問快答：兩個注區挑一個，答完給出莊家優勢的實際差距">
</p>
...
quiz.js               快問快答的出題邏輯（不碰 DOM）
app.js                介面：檯面、下注、結算、統計、實驗室、教學導覽、快問快答
sw.js                 Service Worker：離線快取
manifest.webmanifest  PWA 設定
icons/                PWA 圖示（192／512／maskable／apple-touch）
test/bets.test.js     窮舉 216 種結果驗證每個注區
test/quiz.test.js     驗證出題邏輯：標成正解的必須真的是正解
.github/workflows/    測試與 Pages 自動部署
```

機率模型與出題邏輯獨立成 `bets.js`、`quiz.js`，完全不碰 DOM，
所以測試載入的就是網站實際跑的那份程式碼。
...
- 四十萬局實測返還率收斂到理論值
- 快問快答：每題剛好一個正解、選項不重複、「判輸贏」的答案與實際結算一致、
  「挑注區」不會出兩個一樣好的選項（否則等於在猜）、改了賠率表題目跟著改
- 每個注區的標準差與窮舉結果一致（一賠一的注 sd≈1，圍骰超過 12）
- 一晚的展望：期望值精確、分位數有序、本金夠大就不會破產、同種子可重現、
  注區越差期望成本越高（且比例等於莊家優勢之比）
...
骰寶跟 21 點、撲克不一樣：**它沒有技術可言**。沒有任何決策能改善期望值，
你唯一能決定的只有押哪一格、押多久。所以「練習」是有天花板的——
知道大小／二骰組合是 2.78%、其他都更差，就到頂了。
